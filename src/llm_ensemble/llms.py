...
def answer_questionnaire_llm_wrapper(arguments):
    ...
    return answer_questionnaire_llm(llm, prompts, print_answer)
...
class LLMs:
    ...
    def answer_questionnaire(self, prompts: List[str], print_answer: bool = False):
        answers = []
        for llm in self.llm_instances.keys():
            llm_client = self.llm_instances[llm].get("client")
            ...
            answer, _ = answer_questionnaire_llm(llm_client, prompts, print_answer)

            answers.append({llm: answer})

        return answers

    def answer_questionnaire_parallel(self, prompts: List[str], print_answer: bool = False):

        llm_questionnaire_arguments = tuple(
            (self.llm_instances[x].get('client'), prompts, print_answer)
